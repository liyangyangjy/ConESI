...
def main():
    ...
    #=========================================================================================================================
    #model_directory="/....../scripts/MoLFormer-XL-both-10pct"
    #model_smiles = AutoModel.from_pretrained(model_directory, deterministic_eval=True, trust_remote_code=True)
    ...
    embeddings_results_anchor_enzy = []
    embeddings_results_positive_smiles = []
    embeddings_results_negative_smiles = []
    for i in tqdm(range(ESP_train_df.shape[0]), desc="Processing Sequences"):
        ...
        seq_anchor_enzy = ESP_train_df['anchor_sequence'].iloc[i]
        seq_positive_smiles = ESP_train_df['positive_smiles'].iloc[i]
        seq_negative_smiles = ESP_train_df['negative_smiles'].iloc[i]
        if len(seq_anchor_enzy) < 5500:
            tuple_sequence = tuple(['anchor',seq_anchor_enzy])
            ...
            embeddings_results_anchor_enzy.append(one_seq_embeddings)
            del one_seq_embeddings
            torch.cuda.empty_cache()
            torch.cuda.ipc_collect()
            # the positive smiles embeddings
            positive_smiles_list = []
            positive_smiles_list.append(Chem.CanonSmiles(seq_positive_smiles)) # build a summarize list variable including all the sequence information
            # employ ESM model for converting and save the converted data in csv format
            one_seq_embeddings = MolFormer_embedding(model_smiles, tokenizer, positive_smiles_list,device)
            embeddings_results_positive_smiles.append(one_seq_embeddings)
            del one_seq_embeddings
            torch.cuda.empty_cache()
            torch.cuda.ipc_collect()
            # the negative smiles embeddings
            negative_smiles_list = []
            negative_smiles_list.append(Chem.CanonSmiles(seq_negative_smiles)) # build a summarize list variable including all the sequence information
            # employ ESM model for converting and save the converted data in csv format
            one_seq_embeddings = MolFormer_embedding(model_smiles, tokenizer, negative_smiles_list,device)
            embeddings_results_negative_smiles.append(one_seq_embeddings)
            del one_seq_embeddings
            torch.cuda.empty_cache()
            torch.cuda.ipc_collect()
          

    output_file_anchor_enzy = args.filename.split('.')[0] + '_anchor_enzy.pt'
    output_file_positive_smiles = args.filename.split('.')[0] + '_positive_smiles.pt'
    output_file_negative_smiles = args.filename.split('.')[0] + '_negative_smiles.pt'
    embeddings_results_anchor_enzy_torch = torch.cat(embeddings_results_anchor_enzy, dim=0)
    torch.save(embeddings_results_anchor_enzy_torch, output_file_anchor_enzy)

    embeddings_results_positive_smiles_torch = torch.cat(embeddings_results_positive_smiles, dim=0)
    torch.save(embeddings_results_positive_smiles_torch, output_file_positive_smiles)

    embeddings_results_negative_smiles_torch = torch.cat(embeddings_results_negative_smiles, dim=0)
    torch.save(embeddings_results_negative_smiles_torch, output_file_negative_smiles)
